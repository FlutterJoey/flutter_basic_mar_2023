...
/// A function that will be called whenever a new [StoryBeatEvent] is generated.
typedef StoryBeatListener = FutureOr<void> Function(StoryBeatEvent);

/// A function that will be called whenever a new [MapUpdateEvent] is generated.
typedef MapUpdateEventListener = FutureOr<void> Function(MapUpdateEvent);

/// An event containing the current state of the scene, the current monsters
/// and the current characters.
class MapUpdateEvent {
  /// The monsters with their current position
  final List<MonsterCharacter> monsters;
  /// The characters with their current position
  final List<Character> characters;
  /// The scene of the game as initially provided
  final Scene scene;
...
/// A generator that will produce a story with [StoryBeatEvent]s which you can
/// follow use [MapUpdateEvent]s
///
/// Requires a scene to be set in order to run.
class HorrorStoryGenerator {
  ...
  /// Slears the scene and stops any generation.
  void stop() {
    _timer?.cancel();
    _scene = null;
    _monsters.clear();
  ...
  /// Starts the generation, requires a scene to be set.
  ///
  /// Generates a scene where randomizers determine the fate of
  /// the given characters and monsters from the scene.
  void generate({
    required Duration speed,
  }) {
    assert(_scene != null, 'A scene needs to be set before generate is ran');
    _monsters = _scene!.monsters.map((e) => MonsterCharacter(e)).toList();
    _actors = _scene!.characters
    ...
    _timer = Timer.periodic(speed, _tick);
  }

  String getMap() {
    var result = List<List<String>>.generate(_scene!.length,
        (index) => List<String>.generate(_scene!.width, (index) => ' '));
  ...
  List<MonsterCharacter> _getVisibleMonsters(BaseCharacter character) {
    return _monsters
        .where(
          (element) =>
              element.isVisibleInScene(_scene!, character) &&
              element != character,
        )
        .toList()
      ..sort((a, b) => a
    ...
    for (var actor in _actors) {
      ...
      if (interactableActors.isNotEmpty) {
        ...
        if (actor.archetype == Archetype.angry ||
          ...
          actor._position = _randomNextPoint(actor._position, _scene!);
          other._position = _randomNextPoint(other._position, _scene!);
